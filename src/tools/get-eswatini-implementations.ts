/**
 * get_eswatini_implementations — Find Eswatini statutes implementing a specific EU directive/regulation
...
export interface GetEswatiniImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface EswatiniImplementationResult {
  document_id: string;
...
export async function getEswatiniImplementations(
  db: InstanceType<typeof Database>,
  input: GetEswatiniImplementationsInput,
): Promise<ToolResponse<EswatiniImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as EswatiniImplementationResult[];
