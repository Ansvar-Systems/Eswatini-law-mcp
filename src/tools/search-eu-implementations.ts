/**
 * search_eu_implementations — Search EU directives/regulations that have Eswatini implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_eswatini_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as eswatini_statute_count
  ...
  if (input.has_eswatini_implementation) {
    sql += ' HAVING eswatini_statute_count > 0';
  }
