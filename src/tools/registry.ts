/**
 * Tool registry for Eswatini Law MCP Server.
...
import { getEswatiniImplementations, type GetEswatiniImplementationsInput } from './get-eswatini-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including Eswatini Law (National Council for Law Reporting). ' +
    ...
    'Call this FIRST when you need to understand what Eswatini legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Eswatini statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (article) from an Eswatini proclamation or regulation. ' +
  ...
  },
  {
    ...
    description:
      'Validate an Eswatini legal citation against the database — zero-hallucination check. ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Eswatini statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Eswatini?" ' +
  ...
  },
  {
    ...
    description:
      'Format a Eswatini legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether an Eswatini proclamation or provision is currently in force, amended, or repealed. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get the EU/international legal basis that a Eswatini statute aligns with or references. ' +
      'Eswatini is not an EU member but several Eswatini laws were influenced by EU regulations ' +
    ...
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Eswatini statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_eswatini_implementations',
    description:
      'Find all Eswatini statutes that align with or implement a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Eswatini statutes. ' +
      'Note: Eswatini implements international standards through domestic legislation influenced by international frameworks.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that have Eswatini implementing/aligning legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_eswatini_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents with Eswatini aligning legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU/international legal basis for a SPECIFIC provision within a Eswatini statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU/international alignment status for a Eswatini statute or provision. ' +
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_eswatini_implementations':
          result = await getEswatiniImplementations(db, args as unknown as GetEswatiniImplementationsInput);
          break;
