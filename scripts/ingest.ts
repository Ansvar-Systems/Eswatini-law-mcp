#!/usr/bin/env tsx
/**
 * Eswatini Law MCP -- Census-Driven Ingestion Pipeline (PDF-based)
 *
 * Reads data/census.json and fetches + parses every ingestable Act
 * from laweswatini.com. Eswatini proclamations are published as PDFs.
 *
 * Pipeline per act:
 ...
 *
 * Data source: laweswatini.com (Federal Negarit Gazette)
...
async function downloadPdf(url: string, outputPath: string): Promise<{ status: number; size: number }> {
  ...
  const response = await fetch(url, {
    headers: {
      'User-Agent': 'eswatini-law-mcp/1.0 (https://github.com/Ansvar-Systems/eswatini-law-mcp; hello@ansvar.ai)',
    },
    redirect: 'follow',
  });
...
async function main(): Promise<void> {
  ...
  console.log('Eswatini Law MCP -- Ingestion Pipeline (PDF-based)');
  console.log('===================================================\n');
  console.log(`  Source: laweswatini.com (Federal Negarit Gazette)`);
  ...
  console.log(`\n  Source:      laweswatini.com (Federal Negarit Gazette PDFs)`);
