#!/usr/bin/env tsx
/**
 * Drift detection for Eswatini Law MCP.
 *
 * Checks if upstream Eswatini Law content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Eswatini Law MCP — Drift Detection');
  console.log('================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'eswatini-law-mcp/1.0 drift-detect' },
      });
