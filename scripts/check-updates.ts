#!/usr/bin/env tsx
/**
 * Eswatini Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'http://laweswatini.com';
const PORTAL_NAME = 'Eswatini Law';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/eswatini-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Eswatini Law MCP — Data Freshness Check');
